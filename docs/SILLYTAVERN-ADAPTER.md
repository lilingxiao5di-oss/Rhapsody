...
2. 所有 top-bar drawer 的打开、关闭、定位和移动端行为通过回归测试。
3. 窄屏自动回退原生布局。
4. 功能与快捷键不因视觉位置变化而改变。
...
export async function disable() {
  ...
  delete body.dataset.rhapsodyPerformance;
...
- 活动上限 30 fps，低性能档 20 fps。
- DPR 上限 1.5，低性能档 1。
- 指针视差上限 6px，低性能档 2px。
- 页面隐藏时停止；窗口恢复时按需重绘。
- 无输入、无主题过渡、无有限时长环境 tween 时停止 RAF。
- `prefers-reduced-motion: reduce`、移动窄屏、WebGL 初始化失败或 context lost 时，切换为静态 CSS 光场。
...
- SillyTavern 原生发送、编辑、删除、抽屉、弹窗和移动端流程完整可用。
