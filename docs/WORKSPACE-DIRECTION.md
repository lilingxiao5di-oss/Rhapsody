...
这些尺寸是视觉目标，不代表创建新的业务容器。实现时优先给原生容器重新设定宽度、间距、背景和边界；只为 Rhapsody 自己拥有的设置 UI、环境 Canvas 与可选序曲创建插件节点。纯视觉 V1 不新建伪侧栏或上下文检查器；Figma 骨架与原生 DOM 的具体取舍以 `SILLYTAVERN-ADAPTER.md` 为准。
...
- 序曲、播放频率、OGL、视差以及完整 / 减少 / 关闭动态效果属于插件配置。
- 右侧预览只表达视觉结果，不承诺额外功能。
...
- 不设置自动低性能档；效果无法稳定满足帧时间目标时应优化实现或减少无意义工作。
...
2. 完成 `#sheld`、顶栏、原生导航表面、消息区和输入区的纯 CSS 视觉适配。
3. 实现欢迎态与章节过渡的受控诗性排版。
4. 实现插件设置中心和本地配置持久化。
5. 接入 OGL 环境层，并先完成暂停、降级和销毁机制。
6. 最后接入可选序曲，不让序曲阻塞 SillyTavern 自身初始化。
7. 通过兼容测试后，再单独评估工作区布局实验与只读 Context Inspector。
